...
class ObjectFactory {
    ...
    createGround(type: string, x: number, y: number, length: number, height: number): object{
    ...
    createObject(type: string, x: number, y: number, length: number, height: number, rotation: number): object {
        const options = { };
